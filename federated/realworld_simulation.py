...
from Enco.causal_graphs.graph_real_world import load_graph_file
import pandas as pd
...
if __name__ == '__main__':
    ...
    for f in files:
        ...
        pd.DataFrame(data_obs).to_csv('asia.csv', index=False)
        np.save('asia_adj.npy', original_adjacency_mat)
        # data_obs = np.loadtxt('../data/asia_datasets/real_data_7466.csv', dtype=np.float32, delimiter=',', unpack=False)
        ...
        feature_missing_dict = {0: [0,1,2,3,4,5], 1: [0,6,7,8,9,10], 2: [1,2,3,4,5,6],3:[0,1,7,8,9,10],4:[2,3,4,5,6,7],5:[0,1,2,8,9,10],6:[3,4,5,6,7,8],7:[0,1,2,3,9,10],8:[4,5,6,7,8,9],9:[0,1,2,3,4,10]}
        num_vars_dict = {0:5,1:5,2:5,3:5,4:5,5:5,6:5,7:5,8:5,9:5}
