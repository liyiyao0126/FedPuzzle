...
from client.causal_graphs.graph_definition import CausalDAG, CausalVariable
from client.causal_graphs.variable_distributions import get_random_categorical
from client.causal_graphs.graph_utils import edges_to_adj_matrix
...
def generate_random_graph(variable_names, dist_func, edge_prob, connected=False, max_parents=-1, num_latents=0, **kwargs):
    ...
    #
    #     while True:
    #         any_cycle_removed = False
    #         for node in range(n):
    #             if not visited[node]:
    #                 if dfs(graph, node, visited, rec_stack):
    #                     any_cycle_removed = True
    #         if not any_cycle_removed:
    #             break
    #
    # remove_cycles(adj_matrix)
