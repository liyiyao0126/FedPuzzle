...
from client.causal_graphs.graph_utils import adj_matrix_to_edges, edges_or_adj_matrix, sort_graph_by_vars, get_node_relations
from client.causal_graphs.variable_distributions import ProbDist, ConstantDist, CategoricalDist, DiscreteProbDist, ContinuousProbDist
...
class CausalDAGDataset(CausalDAG):
    ...
    def __init__(self, adj_matrix, data_obs, data_int, latents=None, exclude_inters=None):
        ...
        variables = [CausalVariable(str(i), new_dist(i)) for i in range(adj_matrix.shape[0])]
        super().__init__(variables=variables, adj_matrix=adj_matrix, latents=latents)
        # self.data_obs = data_obs[self.sorted_idxs][..., self.sorted_idxs]
        self.data_obs = data_obs[:,self.sorted_idxs]  # Observational dataset, shape [num_samples, num_vars]
        # print("data_obs shape:", self.data_obs.shape)
